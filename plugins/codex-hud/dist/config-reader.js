...
function buildSentinelPaths(claudeDir, claudeConfigJsonPath, cwd) {
    ...
    if (cwd) {
        paths.push(cwd, path.join(cwd, '.codex'), path.join(cwd, '.codex', 'rules'), path.join(cwd, '.mcp.json'), path.join(cwd, '.codex', 'settings.json'), path.join(cwd, '.codex', 'settings.local.json'), path.join(cwd, 'AGENTS.md'));
    }
    return paths;
}
...
function computeConfigCountsFresh(cwd) {
    ...
    // === USER SCOPE ===
    if (fs.existsSync(path.join(claudeDir, 'AGENTS.md'))) {
        claudeMdCount++;
    }
    // ~/.claude/rules/*.md
    ...
    const projectClaudeDir = cwd ? path.join(cwd, '.codex') : null;
    ...
    if (cwd) {
        ...
        if (!projectClaudeOverlapsUserScope && fs.existsSync(path.join(cwd, '.codex', 'AGENTS.md'))) {
            claudeMdCount++;
        }
...
export async function countConfigs(cwd) {
    ...
    const projectClaudeDir = normalizedCwd ? path.join(normalizedCwd, '.codex') : null;
    const projectClaudeOverlapsUserScope = projectClaudeDir
        ? pathsReferToSameLocation(projectClaudeDir, claudeDir)
        : false;
    if (normalizedCwd && !projectClaudeOverlapsUserScope) {
        ruleSentinelPaths.push(...collectRuleDirectorySentinels(path.join(normalizedCwd, '.codex', 'rules')));
    }
