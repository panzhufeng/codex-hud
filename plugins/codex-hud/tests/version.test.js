...
test('_parseClaudeCodeVersion preserves prerelease and build suffixes', () => {
  assert.equal(_parseClaudeCodeVersion('2.1.81 (Codex)\n'), '2.1.81');
  assert.equal(_parseClaudeCodeVersion('2.2.0-beta.1 (Codex)\n'), '2.2.0-beta.1');
  assert.equal(_parseClaudeCodeVersion('Codex 2.2.0-beta.1+abc123'), '2.2.0-beta.1+abc123');
...
test('getClaudeCodeVersion persists cache across process resets under CODEX_HUD_CONFIG_DIR', async () => {
  const tempHome = await mkdtemp(path.join(tmpdir(), 'codex-hud-version-'));
  ...
  try {
    ...
    _setExecFileImplForTests(async () => {
      execCalls += 1;
      return { stdout: '2.2.0-beta.1+abc123 (Codex)\n' };
    });
    ...
    const cachePath = path.join(customConfigDir, 'plugins', 'codex-hud', '.codex-version-cache.json');
...
test('getClaudeCodeVersion refreshes when the resolved Claude binary path changes', async () => {
  const tempHome = await mkdtemp(path.join(tmpdir(), 'codex-hud-version-symlink-'));
  const customConfigDir = path.join(tempHome, '.claude-alt');
  ...
  try {
    ...
    _setExecFileImplForTests(async () => {
      execCalls += 1;
      return { stdout: execCalls === 1 ? '2.1.97 (Codex)\n' : '2.1.104 (Codex)\n' };
    });
...
test('getClaudeCodeVersion refreshes when the Claude binary mtime changes', async () => {
  const tempHome = await mkdtemp(path.join(tmpdir(), 'codex-hud-version-invalidate-'));
  const customConfigDir = path.join(tempHome, '.claude-alt');
  const binaryPath = path.join(tempHome, 'claude');
  const originalHome = process.env.HOME;
  const originalConfigDir = process.env.CODEX_HUD_CONFIG_DIR;
  ...
  try {
    ...
    _setExecFileImplForTests(async () => {
      execCalls += 1;
      return { stdout: execCalls === 1 ? '2.1.81 (Codex)\n' : '2.2.0-beta.1 (Codex)\n' };
    });
...
test('getClaudeCodeVersion executes the resolved binary path', async () => {
  const tempHome = await mkdtemp(path.join(tmpdir(), 'codex-hud-version-windows-'));
  const customConfigDir = path.join(tempHome, '.claude-alt');
...
test('getClaudeCodeVersion uses the Windows wrapper invocation for .cmd binaries', async () => {
  const tempHome = await mkdtemp(path.join(tmpdir(), 'codex-hud-version-wrapper-'));
  const customConfigDir = path.join(tempHome, '.claude-alt');
  const binaryPath = path.join(tempHome, 'claude.cmd');
  const originalHome = process.env.HOME;
  const originalConfigDir = process.env.CODEX_HUD_CONFIG_DIR;
