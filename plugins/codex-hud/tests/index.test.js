...
async function createTempConfigDir(config = {}) {
  const dir = await mkdtemp(path.join(tmpdir(), "codex-hud-index-test-"));
  const pluginDir = path.join(dir, "plugins", "codex-hud");
...
test("index entrypoint runs when executed directly", async () => {
  ...
  try {
    process.env.CODEX_HUD_CONFIG_DIR = dir;
...
test("main includes Codex version in render context only when enabled", async () => {
  let renderedContext;
  let lookupCalls = 0;
...
test("main skips Codex version lookup when disabled", async () => {
  let lookupCalls = 0;
