...
test('getOutputSpeed returns null when output tokens are missing', () => {
  const speed = getOutputSpeed({
    transcript_path: '/tmp/codex-hud-speed-missing.jsonl',
...
test('getOutputSpeed writes cache under CODEX_HUD_CONFIG_DIR by default', async () => {
  const tempHome = await createTempHome();
  ...
  try {
    ...
    const customCacheDir = path.join(customConfigDir, 'plugins', 'codex-hud', 'speed-cache');
    const defaultCacheDir = path.join(tempHome, '.codex', 'plugins', 'codex-hud', 'speed-cache');
