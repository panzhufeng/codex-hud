...
test('getGitStatus builds branchUrl from HTTPS origin remotes', async () => {
  ...
  try {
    ...
    execFileSync('git', ['commit', '--allow-empty', '-m', 'init'], { cwd: dir, stdio: 'ignore' });
    execFileSync('git', ['checkout', '-b', 'feature/test-branch'], { cwd: dir, stdio: 'ignore' });
    execFileSync('git', ['remote', 'add', 'origin', 'https://github.com/example/codex-hud.git'], { cwd: dir, stdio: 'ignore' });

    const result = await getGitStatus(dir);
    assert.equal(result?.branchUrl, 'https://github.com/example/codex-hud/tree/feature%2Ftest-branch');
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});
...
test('getGitStatus builds branchUrl from SSH origin remotes', async () => {
  ...
  try {
    ...
    execFileSync('git', ['commit', '--allow-empty', '-m', 'init'], { cwd: dir, stdio: 'ignore' });
    execFileSync('git', ['checkout', '-b', 'feature/test-branch'], { cwd: dir, stdio: 'ignore' });
    execFileSync('git', ['remote', 'add', 'origin', 'git@github.com:example/codex-hud.git'], { cwd: dir, stdio: 'ignore' });
...
test('getGitStatus does not build branchUrl for non-GitHub HTTPS remotes', async () => {
  ...
  try {
    ...
    execFileSync('git', ['commit', '--allow-empty', '-m', 'init'], { cwd: dir, stdio: 'ignore' });
    execFileSync('git', ['checkout', '-b', 'feature/test-branch'], { cwd: dir, stdio: 'ignore' });
    execFileSync('git', ['remote', 'add', 'origin', 'https://gitlab.com/example/codex-hud.git'], { cwd: dir, stdio: 'ignore' });
...
test('getGitStatus does not build branchUrl for non-GitHub SSH remotes', async () => {
  ...
  try {
    ...
    execFileSync('git', ['commit', '--allow-empty', '-m', 'init'], { cwd: dir, stdio: 'ignore' });
    execFileSync('git', ['checkout', '-b', 'feature/test-branch'], { cwd: dir, stdio: 'ignore' });
    execFileSync('git', ['remote', 'add', 'origin', 'git@gitlab.com:example/codex-hud.git'], { cwd: dir, stdio: 'ignore' });
