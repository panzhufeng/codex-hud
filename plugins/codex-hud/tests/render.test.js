...
async function withDeterministicSpeedCache(fn) {
  const tempConfigDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-render-'));
  ...
  const cachePath = path.join(tempConfigDir, 'plugins', 'codex-hud', 'speed-cache', `${transcriptHash}.json`);

  process.env.CODEX_HUD_CONFIG_DIR = tempConfigDir;
  ...
  try {
    ...
    if (originalConfigDir === undefined) {
      delete process.env.CODEX_HUD_CONFIG_DIR;
    } else {
      process.env.CODEX_HUD_CONFIG_DIR = originalConfigDir;
    }
...
test('renderSessionLine includes Codex version when enabled', () => {
  const ctx = baseContext();
  ctx.stdin.cwd = '/tmp/my-project';
...
test('renderProjectLine includes Codex version when enabled', () => {
  const ctx = baseContext();
  ctx.stdin.cwd = '/tmp/my-project';
  ctx.config.display.showClaudeCodeVersion = true;
  ctx.claudeCodeVersion = '2.1.81';
...
test('label color overrides apply across shared secondary text surfaces', () => {
  ...
  assert.ok(renderEnvironmentLine(ctx)?.includes(`${expected}2 AGENTS.md | 1 rules\x1b[0m`));
...
test('renderEnvironmentLine appends output style after config counts', () => {
  ...
  assert.ok(line?.includes('1 AGENTS.md'));
...
test('renderProjectLine strips control characters from project and branch links', () => {
  ...
  ctx.gitStatus = {
    ...
    ahead: 0,
    behind: 0,
    branchUrl: 'https://github.com/example/codex-hud/tree/feat%2Fname\u0007',
  };
