...
async function getTranscriptCacheFile(configDir) {
  const cacheDir = path.join(configDir, 'plugins', 'codex-hud', 'transcript-cache');
...
// Native percentage tests (Codex v2.1.6+)
...
test('getContextPercent falls through to token-based calculation when used_percentage is 0 but tokens exist', () => {
  // On a fresh session Codex emits used_percentage=0 before the first API
...
test('getUsageFromStdin parses official Codex rate_limits payload', () => {
...
test('parseTranscript does not cache partial results when stream creation fails after file state lookup', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'codex-hud-transcript-cache-'));
  const configDir = path.join(dir, '.claude-test');
  const transcriptPath = path.join(dir, 'stream-failure.jsonl');
  const originalConfigDir = process.env.CODEX_HUD_CONFIG_DIR;
  const cacheDir = path.join(configDir, 'plugins', 'codex-hud', 'transcript-cache');

  process.env.CODEX_HUD_CONFIG_DIR = configDir;
  ...
  try {
    ...
    restoreEnvVar('CODEX_HUD_CONFIG_DIR', originalConfigDir);
    await rm(dir, { recursive: true, force: true });
  }
});
...
test('countConfigs returns outputStyle with project local precedence', async () => {
  ...
  try {
    ...
    await writeFile(
      path.join(homeDir, '.codex', 'settings.local.json'),
...
test('countConfigs uses CODEX_HUD_CONFIG_DIR and matching .json sidecar for user scope', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-home-'));
  const customConfigDir = path.join(homeDir, '.claude-2');
  const originalHome = process.env.HOME;
  const originalConfigDir = process.env.CODEX_HUD_CONFIG_DIR;
  process.env.HOME = homeDir;
  process.env.CODEX_HUD_CONFIG_DIR = customConfigDir;

  try {
    // Default directory should be ignored when CODEX_HUD_CONFIG_DIR is set.
...
test('countConfigs still counts project .codex when cwd is home and CODEX_HUD_CONFIG_DIR points elsewhere', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-home-'));
  const customConfigDir = path.join(homeDir, '.claude-2');
  const originalHome = process.env.HOME;
  const originalConfigDir = process.env.CODEX_HUD_CONFIG_DIR;
...
async function getConfigCacheDir(configDir) {
  return path.join(configDir, 'plugins', 'codex-hud', 'config-cache');
}

test('countConfigs cache: second call uses cache (mtime unchanged)', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-cc-'));
  const originalHome = process.env.HOME;
  process.env.HOME = homeDir;
  ...
  try {
    ...
    const settingsPath = path.join(homeDir, '.codex', 'settings.json');
    ...
    const claudeMdPath = path.join(claudeDir, 'AGENTS.md');
...
test('countConfigs cache: miss on file creation (AGENTS.md appears)', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-cc-'));
  const originalHome = process.env.HOME;
  process.env.HOME = homeDir;
  ...
  try {
    ...
    // Create AGENTS.md — changes the directory mtime
    await writeFile(path.join(homeDir, '.codex', 'AGENTS.md'), 'global', 'utf8');

    const second = await countConfigs();
    assert.equal(second.claudeMdCount, 1, 'Should detect newly created AGENTS.md');
  } finally {
    process.env.HOME = originalHome;
    await rm(homeDir, { recursive: true, force: true });
  }
...
test('countConfigs cache: miss on file deletion (AGENTS.md removed)', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-cc-'));
  const originalHome = process.env.HOME;
  process.env.HOME = homeDir;
  ...
  try {
    ...
    // Delete AGENTS.md — changes the directory mtime
    fs.unlinkSync(path.join(homeDir, '.codex', 'AGENTS.md'));

    const second = await countConfigs();
    assert.equal(second.claudeMdCount, 0, 'Should detect deleted AGENTS.md');
  } finally {
    process.env.HOME = originalHome;
    await rm(homeDir, { recursive: true, force: true });
  }
...
test('countConfigs cache: miss on nested rules additions', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-cc-'));
  const projectDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-proj-'));
  const originalHome = process.env.HOME;
  process.env.HOME = homeDir;

  try {
    await mkdir(path.join(homeDir, '.codex'), { recursive: true });
    await mkdir(path.join(projectDir, '.codex', 'rules', 'nested'), { recursive: true });
    await writeFile(path.join(projectDir, '.codex', 'rules', 'nested', 'one.md'), '# one', 'utf8');

    const first = await countConfigs(projectDir);
    assert.equal(first.rulesCount, 1);

    await writeFile(path.join(projectDir, '.codex', 'rules', 'nested', 'two.md'), '# two', 'utf8');
...
test('countConfigs cache: isolation between different cwds', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-cc-'));
  const projectA = await mkdtemp(path.join(tmpdir(), 'codex-hud-projA-'));
  const projectB = await mkdtemp(path.join(tmpdir(), 'codex-hud-projB-'));
  const originalHome = process.env.HOME;
  process.env.HOME = homeDir;

  try {
    await mkdir(path.join(homeDir, '.codex'), { recursive: true });
    await writeFile(path.join(projectA, 'AGENTS.md'), 'projA', 'utf8');
    // projectB has no AGENTS.md

    const countsA = await countConfigs(projectA);
    const countsB = await countConfigs(projectB);

    assert.equal(countsA.claudeMdCount, 1, 'Project A should have 1 AGENTS.md');
    assert.equal(countsB.claudeMdCount, 0, 'Project B should have 0 AGENTS.md');

    // Verify both get independent caches
    const cacheDir = await getConfigCacheDir(path.join(homeDir, '.codex'));
...
test('countConfigs cache: isolation between different CODEX_HUD_CONFIG_DIRs', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'codex-hud-cc-'));
  ...
  try {
    ...
    await writeFile(path.join(configA, 'AGENTS.md'), 'config-a', 'utf8');
    // configB has no AGENTS.md

    process.env.CODEX_HUD_CONFIG_DIR = configA;
    const countsA = await countConfigs();

    process.env.CODEX_HUD_CONFIG_DIR = configB;
    const countsB = await countConfigs();

    assert.equal(countsA.claudeMdCount, 1, 'Config A should have 1 AGENTS.md');
    assert.equal(countsB.claudeMdCount, 0, 'Config B should have 0 AGENTS.md');
  } finally {
    restoreEnvVar('HOME', originalHome);
    restoreEnvVar('CODEX_HUD_CONFIG_DIR', originalConfigDir);
    await rm(homeDir, { recursive: true, force: true });
...
test('countConfigs cache: malformed cache payload falls back to fresh recompute', async () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(cacheDir, cacheFiles[0]), JSON.stringify({
      key: {
        cwd: null,
        claudeConfigDir: path.join(homeDir, '.codex'),
        sentinels: {
          [path.join(homeDir, '.codex', 'AGENTS.md')]: fs.existsSync(path.join(homeDir, '.codex', 'AGENTS.md'))
            ? { mtimeMs: fs.statSync(path.join(homeDir, '.codex', 'AGENTS.md')).mtimeMs, size: fs.statSync(path.join(homeDir, '.codex', 'AGENTS.md')).size }
            : null,
          [path.join(homeDir, '.codex', 'rules')]: null,
          [path.join(homeDir, '.codex', 'settings.json')]: null,
          [path.join(homeDir, '.codex.json')]: null,
        },
      },
      data: {
        claudeMdCount: 'oops',
